...
assert.match(mount, /box-sizing: border-box/);
assert.match(mount, /max-width: 100%/);
assert.match(mount, /overflow: visible/);
assert.doesNotMatch(mount, /forge-alive-primary-view-067g16a[\s\S]{0,320}overflow: auto/);
...
assert.match(controller, /const requested = new URL\(location\.href\)\.searchParams\.get\('nav'\) \|\| 'inicio';[\s\S]*if \(requested === 'cotizaciones'\) return null;[\s\S]*SUPPORTED_VIEWS\.has\(requested\)/);
assert.ok(controller.includes("SUPPORTED_VIEWS = new Set(['inicio', 'pipeline', 'clientes', 'mas', 'alfred', 'reportes'])"));
assert.equal((controller.match(/requested === 'cotizaciones'/g) || []).length, 1);
assert.equal((controller.match(/document\.documentElement\.setAttribute\('data-forge-alive-static-view-067g16a', requestedView\(\) \|\| 'cotizaciones'\)/g) || []).length, 1);
...
assert.ok(html.includes('pipeline-ui.css?v=067g16f-1'));
